<?php
    ...
    $resultado = $conex->ejecutarConsulta("SELECT nick FROM usuario WHERE nick = ?");

    if($resultado->execute(array($insertarUsuario->getNick()))){

        $fila = $resultado->fetchAll(PDO::FETCH_NUM);

        if (count($fila) >= 1) {
            
            header("Location: /aprendiendoaprogramar.com/registro.php");
        ...
        echo "No se pudo ejecutar el registro, intentelo mas tarde";
    }
